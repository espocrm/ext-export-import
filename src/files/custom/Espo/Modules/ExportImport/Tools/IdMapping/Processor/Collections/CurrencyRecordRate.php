...
class CurrencyRecordRate implements CollectionProcessor
{
    ...
    private function getRowIdMap(Params $params, array $row): ?array
    {
        ...
        $actualRecordId = $params->getActualIdMap()['CurrencyRecord'][$recordId] ?? null;

        if (!$actualRecordId) {
            return null;
        }

        $record = $this->getRateByRecordId($actualRecordId, $date);
        ...
        $this->log->debug(
            'Imported CurrencyRecordRate [' . $id . '] is linked ' .
            'to the CurrencyRecordRate [' . $actualId . '] ' .
            'identified by the recordId [' . $actualRecordId . '] and date [' . $date . '].'
        );
     ...
     */
    private function getRateByRecordId(string $recordId, string $date): ?Entity
    {
        if (!class_exists('\\Espo\\Entities\\CurrencyRecordRate')) {
            return null;
        }

        return $this->entityManager
            ->getRDBRepository('CurrencyRecordRate')
            ->where([
                'recordId' => $recordId,
                'date' => $date,
            ])
